  ...
  $.ajax({
    ...
    if(result.status == 200){
      ...
      for(var i in jsonResult){
        ...
        text +='      <a href="../product_history/assembly_end_print.html?id='+jsonResult[i].order_id+'"><button type="button" class="btn btn-bg-gradient-x-purple-blue">출력</button></a>';
        text +='    </div>';
        text +='  </td>';
        text +='  <td class="text-center">';
        text +='    <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">';
        text +='      <a href="../product_history/assembly_start_detail.html?id='+jsonResult[i].id+'"><button type="button" class="btn btn-warning">상세보기</button></a>';
        text +='    </div>';
        text +='  </td>';
        text +='</tr>';
        ...
        text +='        <td class="text-center">공정시작시간</td>';
      ...
      $("#painting_start_list #open_tr").on("click", function(){
        if($(this).parents("tr").next("tr#sub_tr").css("display") == "table-row"){
          $("#painting_start_list tr#sub_tr").fadeOut(0);
          $(this).parents("tr").css("background-color","#fff");
          $("#painting_start_list #open_tr").text("▼");
          start();
        }else{
          $("#painting_start_list tr").css("background-color","#fff");
...
function paging(end, start, total){
  ...
  if (total_paging_cnt == 0 || total_paging_cnt == 1 || pre_no == 0)
  {
    ...
    text +='<a class="page-link" href="./assembly_start.html?page_no='+pre_no+'&search_text='+search_text+'&start_date='+start_date+'&end_date='+end_date+'&asset_id='+asset_id+'&sort='+sort+'&order='+order+'&sort_select='+$("#basicSelect").val()+'" aria-label="Previous">';
  ...
  for( var k = paging_init_num; k <= paging_end_num; k++){
    ...
    {
      text +='<li class="page-item active"><a class="page-link" href="./assembly_start.html?page_no='+k+'&search_text='+search_text+'&start_date='+start_date+'&end_date='+end_date+'&asset_id='+asset_id+'&sort='+sort+'&order='+order+'&sort_select='+$("#basicSelect").val()+'">'+k+'</a></li>';
    }else{
      text +='<li class="page-item"><a class="page-link" href="./assembly_start.html?page_no='+k+'&search_text='+search_text+'&start_date='+start_date+'&end_date='+end_date+'&asset_id='+asset_id+'&sort='+sort+'&order='+order+'&sort_select='+$("#basicSelect").val()+'">'+k+'</a></li>';
    }
  }
  if (total_paging_cnt == 0 || total_paging_cnt == 1 || next_no > total_paging_cnt)
  {
    ...
    text +='<li class="page-item">';
    text +='  <a class="page-link" href="./assembly_start.html?page_no='+next_no+'&search_text='+search_text+'&start_date='+start_date+'&end_date='+end_date+'&asset_id='+asset_id+'&sort='+sort+'&order='+order+'&sort_select='+$("#basicSelect").val()+'" aria-label="Next">';
...
$("#search_btn").on("click", function(){
  location.href="../product_history/assembly_start.html?start_date="+$("#start_date").val()+"&end_date="+$("#end_date").val()+"&search_text="+$("#search_text").val()+"&asset_id="+$("#asset_id").val();
});
